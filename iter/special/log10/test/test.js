...
tape( 'main export is a function', function test( t ) {
	...
	t.equal( typeof iterLog10, 'function', 'main export is a function' );
	t.end();
});
...
tape( 'the function returns an iterator protocol-compliant object (value+done)', function test( t ) {
	...
	it = iterLog10( createIterator( values ) );
	t.equal( it.next.length, 0, 'has zero arity' );
...
tape( 'the function returns an iterator protocol-compliant object which evaluates the common logarithm for each iterated value', function test( t ) {
	var expected;
	var it;
	var N;
	var x;
	...
	N = 101;
	it = iterLog10( linspace( 0.0, 1000.0, N ) );
	t.equal( it.next.length, 0, 'has zero arity' );

	x = linspace( 0.0, 1000.0, N );
	for ( i = 0; i < N; i++ ) {
		r = it.next();
		expected = log10( x.next().value );
...
tape( 'the function returns an iterator protocol-compliant object which evaluates the common logarithm for each iterated value (value+done)', function test( t ) {
	var expected;
	var actual;
	var values;
	var it;
	...
	expected = [
		{
			'value': log10( values[ 0 ] ),
			'done': false
		},
		{
			'value': log10( values[ 1 ] ),
			'done': false
		},
		{
			'value': log10( values[ 2 ] ),
			'done': false
		},
		{
			'value': log10( values[ 3 ] ),
			'done': true
		}
	];
