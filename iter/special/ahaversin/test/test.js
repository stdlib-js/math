...
var ahaversin = require( './../../../../base/special/ahaversin' );
...
tape( 'main export is a function', function test( t ) {
	...
	t.equal( typeof iterAhaversin, 'function', 'main export is a function' );
	t.end();
});
...
tape( 'the function returns an iterator protocol-compliant object', function test( t ) {
	...
	it = iterAhaversin( uniform( 0.0, 1.0 ) );
...
tape( 'the function returns an iterator protocol-compliant object (value+done)', function test( t ) {
	...
	values = [ 0.0, 0.5, 1.0 ];
	it = iterAhaversin( createIterator( values ) );
	t.equal( it.next.length, 0, 'has zero arity' );
...
tape( 'the function returns an iterator protocol-compliant object which computes the inverse half-value versed sine of each iterated value', function test( t ) {
	var expected;
	var it;
	var N;
	var x;
	...
	N = 101;
	it = iterAhaversin( linspace( 0.0, 1.0, N ) );
	t.equal( it.next.length, 0, 'has zero arity' );

	x = linspace( 0.0, 1.0, N );
	for ( i = 0; i < N; i++ ) {
		r = it.next();
		expected = ahaversin( x.next().value );
...
tape( 'the function returns an iterator protocol-compliant object which computes the inverse half-value versed sine of each iterated value (value+done)', function test( t ) {
	var expected;
	var actual;
	var values;
	var it;
	...
	expected = [
		{
			'value': ahaversin( values[ 0 ] ),
			'done': false
		},
		{
			'value': ahaversin( values[ 1 ] ),
			'done': false
		},
		{
			'value': ahaversin( values[ 2 ] ),
			'done': false
		},
		{
			'value': ahaversin( values[ 3 ] ),
			'done': true
		}
	];
