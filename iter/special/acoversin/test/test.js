...
tape( 'main export is a function', function test( t ) {
	...
	t.equal( typeof iterAcoversin, 'function', 'main export is a function' );
	t.end();
});
...
tape( 'the function returns an iterator protocol-compliant object', function test( t ) {
	...
	it = iterAcoversin( uniform( 0.0, 2.0 ) );
...
tape( 'the function returns an iterator protocol-compliant object (value+done)', function test( t ) {
	...
	values = [ 0.0, 0.5, 1.0 ];
	it = iterAcoversin( createIterator( values ) );
	t.equal( it.next.length, 0, 'has zero arity' );
...
tape( 'the function returns an iterator protocol-compliant object which computes the inverse coversed sine of each iterated value', function test( t ) {
	var expected;
	var it;
	var N;
	var x;
	...
	N = 101;
	it = iterAcoversin( linspace( 0.0, 2.0, N ) );
	t.equal( it.next.length, 0, 'has zero arity' );

	x = linspace( 0.0, 2.0, N );
	for ( i = 0; i < N; i++ ) {
		r = it.next();
		expected = acoversin( x.next().value );
...
tape( 'the function returns an iterator protocol-compliant object which computes the inverse coversed sine of each iterated value (value+done)', function test( t ) {
	var expected;
	var actual;
	var values;
	var it;
	...
	expected = [
		{
			'value': acoversin( values[ 0 ] ),
			'done': false
		},
		{
			'value': acoversin( values[ 1 ] ),
			'done': false
		},
		{
			'value': acoversin( values[ 2 ] ),
			'done': false
		},
		{
			'value': acoversin( values[ 3 ] ),
			'done': true
		}
	];
