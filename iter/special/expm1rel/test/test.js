...
var expm1rel = require( './../../../../base/special/expm1rel' );
...
tape( 'main export is a function', function test( t ) {
	...
	t.equal( typeof iterExpm1rel, 'function', 'main export is a function' );
	t.end();
});
...
tape( 'the function returns an iterator protocol-compliant object', function test( t ) {
	...
	it = iterExpm1rel( uniform( -50.0, 50.0 ) );
...
tape( 'the function returns an iterator protocol-compliant object (value+done)', function test( t ) {
	...
	it = iterExpm1rel( createIterator( values ) );
	t.equal( it.next.length, 0, 'has zero arity' );
...
tape( 'the function returns an iterator protocol-compliant object which evaluates the relative error exponential for each iterated value', function test( t ) {
	var expected;
	var rand;
	var it;
	var N;
	...
	for ( i = 0; i < N; i++ ) {
		r = it.next();
		expected = expm1rel( x.next().value );
...
tape( 'the function returns an iterator protocol-compliant object which evaluates the relative error exponential for each iterated value (value+done)', function test( t ) {
	var expected;
	var actual;
	var values;
	var it;
	...
	expected = [
		{
			'value': expm1rel( values[ 0 ] ),
			'done': false
		},
		{
			'value': expm1rel( values[ 1 ] ),
			'done': false
		},
		{
			'value': expm1rel( values[ 2 ] ),
			'done': false
		},
		{
			'value': expm1rel( values[ 3 ] ),
			'done': true
		}
	];
