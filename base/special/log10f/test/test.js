...
tape( 'main export is a function', function test( t ) {
	...
	t.strictEqual( typeof log10f, 'function', 'main export is a function' );
	t.end();
});
...
tape( 'the function returns `-infinity` if provided `+-0`', function test( t ) {
	t.strictEqual( log10f( 0.0 ), NINF, 'returns expected value' );
	t.strictEqual( log10f( -0.0 ), NINF, 'returns expected value' );
	t.end();
});

tape( 'the function returns `+infinity` if provided `+infinity`', function test( t ) {
	t.strictEqual( log10f( PINF ), PINF, 'returns expected value' );
	t.end();
});
